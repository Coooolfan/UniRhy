...
1. **服务端 Ping/Pong 活性检测**（D1）：替换基于 `lastNtpResponseAtMs` 的 stale 判定；
2. **回前台自愈修复**（独立小改动，任何路线下都需要）：
   - resume/`visibilitychange` 时强制校验并重建连接；
   - 修复 `awaitingSyncRecovery` 悬空卡死（发送失败或重连后重置并重发）；
   - 409 `VERSION_CONFLICT` 恢复改走 HTTP 拉取队列快照，不依赖 WS 存活；
3. **Android 原生播放内核技术方案**（D2）：插件结构、Kotlin 协议客户端范围、与 TS 层的接口切分、迁移策略；
4. **iOS 原生播放层立项**（D4，后置）。
