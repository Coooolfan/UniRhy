...
> [!NOTE]
> 当前版本号规范使用点分隔预发布序号（如 `0.1.0-rc.1`）。仓库中存在连字符分隔的历史 tag（如 `0.0.1-alpha-1`），两种格式在判定级别时视为等价，`git-tags.sh` 对两者均可识别与排序。
...
7. 如果翻遍所有 tag 仍未找到符合条件的历史 tag（如首个正式版本发布时），则变更范围为"仓库初始提交至当前目标 tag 的全部变化"，此时省略 Full Changelog 的 compare 链接。
