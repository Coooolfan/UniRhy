...
| Term ID           | 中文标准词 | English Canonical | 可接受别名           | 禁用词                 | 替换建议                | 定义                                   | 适用范围             | 示例                                                                     |
| ----------------- | ---------- | ----------------- | -------------------- | ---------------------- | ----------------------- | -------------------------------------- | -------------------- | ------------------------------------------------------------------------ |
| `music.album`     | 专辑       | Album             | 唱片（非技术场景）   | -                      | -                       | 专辑聚合实体，包含一组曲目。           | UI / API / DB / 文档 | 中：该专辑收录 10 条曲目。<br>EN: This album contains 10 tracks.         |
...
| 概念           | 非标写法                   | 统一后标准词                                      | 禁用范围                                                    | 处理规则                                                                      |
| -------------- | -------------------------- | ------------------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Playlist 实体  | `SongList`、歌单、播放列表 | 歌单 / Playlist                                   | 实体命名（代码 / API / 文档）                               | 代码与 API 统一 `Playlist`，UI 中文统一“歌单”。                               |
| Recording 实体 | _按分域术语执行_           | UI/文档：曲目 / Track；API/模型：录音 / Recording | UI/文档禁用 `recording` / 录音；API/模型禁用 `track` / 曲目 | 按分域术语执行：UI/文档使用 `Track` / 曲目，API/模型使用 `Recording` / 录音。 |
| 存储配置单元   | 存储点、存储节点、Provider | 存储节点 / Storage Node                           | 实体命名（代码 / API / 文档）                               | 具体配置单元统一“存储节点”，`Provider` 仅保留抽象类型语义。                   |
| 内容浏览入口   | 资料库、阅览室、Albums     | 资料库 / Library                                  | 导航与标题（UI / 文档）                                     | 导航与标题统一“资料库 / Library”，`Albums` 仅用于内部路由片段。               |
| 认证字段 email | 邮箱、电子邮箱、`username` | 邮箱 / Email                                      | 字段命名（API / DB / 文档）                                 | 字段名统一 `email`，禁止以 `username` 承载邮箱语义。                          |
...
| recording/录音（仅限制UI/文档） | 曲目 / Track            | “recording/录音”仅可用于 API/模型 术语，不得用于UI/文档。 |
| track/曲目（仅限制API/模型）    | 录音 / Recording        | “track/曲目”仅可用于 UI/文档 术语，不得用于API/模型。     |
