...
2. 分页调用 `./skills/release-note-creator/scripts/git-tags.sh --pagesize <N> --offset <M>` 获取历史 tag 列表。
...
流程：
1. 选定为正式版本（Release）
2. 分页调用 `./skills/release-note-creator/scripts/git-tags.sh` 获取历史 tag 列表
